export function patchDodgeChance({ patch }) {
    patch(Player, "computeCombatStats").before(function () {
        const dod = this.game.modifiers.getValue('WTM:dodgeChancePer10000EnemyAccuracyRating', ModifierQuery.EMPTY);
        if (dod) {
            if (this.target.id !== this.targetForDodge) {
                ...
                const extradodgechance = Math.floor(this.target.stats._accuracy * dod / 10) / 1000;
